...
/*   Updated: 2025/03/10 14:17:51 by vide-sou         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	ft_start_table(char **av, t_table *table)
{
	if (ft_atol(av[1]) <= 0 || ft_atol(av[2]) <= 0 || ft_atol(av[3]) <= 0
		|| ft_atol(av[4]) <= 0 || ft_atol(av[4]) <= 0)
	{
		write(2, "Nenhum valor pode ser negativo ou zero\n", 40);
		exit(-1);
	}
...
int	main(int ac, char **av)
{
	...
	while (index < table.philosophers_number)
	{
		pthread_create(&table.thread[index], NULL, ft_philo_routine,
			&(table.philosophers_list[index]));
		index++;
	}
