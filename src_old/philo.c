...
/*   Updated: 2025/03/10 14:17:48 by vide-sou         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	*ft_philo_routine(void *arg)
{
	...
	while (ft_philo_is_alived(philo) && current_time
		- philo->started_at < ft_get_lifetime(philo) + philo->time_to_die
		&& philo->hungry_size != 0)
	{
		if (sleep)
		{
			ft_get_time(philo->time_to_sleep, &current_time);
			printf("%ld %d has sleeping\n", current_time - philo->started_at,
				ft_get_id(philo));
			sleep = 0;
		}
		...
		else if (ft_get_action(philo) == GETTING)
		{
			printf("%ld %d has getted 1 fork\n", current_time
				- philo->started_at, ft_get_id(philo));
			printf("%ld %d has getted 1 fork\n", current_time
				- philo->started_at, ft_get_id(philo));
			fork += 2;
